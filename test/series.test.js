...
/* ══ o saguão de séries ═══════════════════════════════════════════════════
   A quinta parede outra vez, agora sobre outra tabela. O modo de falhar é o
   mesmo e é o pior deste produto: nada quebra. Um clube fechado que vaza para
   um ranking de rede produz um cartaz a mais numa parede bonita, e ninguém
   descobre até a pessoa errada reconhecer a série. */

const lobbySeries = require('../lobbySeries');

/** Liga o que a sala empresta à rede, direto no banco. */
async function lend(club, { charts = 0, reviews = 0 } = {}) {
  await db
    .prepare('UPDATE clubs SET show_charts = ?, show_reviews = ? WHERE id = ?')
    .run(charts, reviews, club.id);
  lobbySeries.invalidate();
}

test('um clube fechado que não empresta não existe no saguão de séries', async () => {
  const p = await kit.signIn();
  const club = await kit.makeClub({ owner: p.id, visibility: 'private' });
  const s = show('Série Secreta');

  await req(
    'PUT', at(club, `/shows/${s.id}/1/1`),
    { showTitle: s.title, showPoster: s.poster, genre: 'Drama', scores: scoresFor('Drama', 10) },
    p.cookie
  );
  await lend(club, { charts: 0 });

  const { body } = await req('GET', '/api/lobby/series');
  assert.ok(
    !body.wall.some(w => w.title === 'Série Secreta'),
    'a parede carregou uma série de uma sala que não empresta'
  );
  assert.ok(!body.podium.some(x => x.title === 'Série Secreta'));
  assert.ok(!body.active.some(c => c.slug === club.slug));
});

test('emprestar as contas põe a série na parede, mas não o texto assinado', async () => {
  const p = await kit.signIn();
  const club = await kit.makeClub({ owner: p.id, visibility: 'private' });
  const s = show('Série Emprestada');

  await req(
    'PUT', at(club, `/shows/${s.id}/1/1`),
    {
      showTitle: s.title, showPoster: s.poster, genre: 'Drama',
      scores: scoresFor('Drama', 9), comment: 'o melhor episódio do ano',
    },
    p.cookie
  );
  /* Só `show_charts`: a sala entra nas CONTAS e não nas fichas assinadas. */
  await lend(club, { charts: 1, reviews: 0 });

  const { body } = await req('GET', '/api/lobby/series');
  assert.ok(body.wall.some(w => w.title === 'Série Emprestada'), 'a parede deveria ter a série');
  assert.equal(
    body.feature?.movieTitle, undefined,
    'o destaque tem nome e texto de gente: exige a parede mais alta'
  );
});

test('só episódio com nota conta como avaliação', async () => {
  const p = await kit.signIn();
  const club = await kit.makeClub({ owner: p.id, visibility: 'public' });
  const s = show();
  const base = { showTitle: s.title, showPoster: s.poster, genre: 'Drama' };

  const antes = (await req('GET', '/api/lobby/series')).body.counts.reviews;
  // Visto e não avaliado: o gesto mais barato do produto não pode inflar o número.
  await req('PUT', at(club, `/shows/${s.id}/1/1`), base, p.cookie);
  lobbySeries.invalidate();
  const meio = (await req('GET', '/api/lobby/series')).body.counts.reviews;
  assert.equal(meio, antes, 'marcar visto não é avaliar');

  await req('PUT', at(club, `/shows/${s.id}/1/2`), { ...base, quick: 8 }, p.cookie);
  lobbySeries.invalidate();
  const depois = (await req('GET', '/api/lobby/series')).body.counts.reviews;
  assert.equal(depois, antes + 1);
});

test('a curva por temporada é derivada dos mesmos episódios que a média', async () => {
  const p = await kit.signIn();
  const club = await kit.makeClub({ owner: p.id, visibility: 'public' });
  const s = show();
  const base = { showTitle: s.title, showPoster: s.poster, genre: 'Drama' };

  await req('PUT', at(club, `/shows/${s.id}/1/1`), { ...base, quick: 6 }, p.cookie);
  await req('PUT', at(club, `/shows/${s.id}/1/2`), { ...base, quick: 8 }, p.cookie);
  await req('PUT', at(club, `/shows/${s.id}/2/1`), { ...base, quick: 10 }, p.cookie);

  const { body } = await req('GET', `/api/lobby/show/${s.id}`);
  assert.equal(body.episodes, 3);
  assert.equal(body.average, 8, 'a média da série é a dos episódios');
  assert.deepEqual(
    body.seasons.map(x => [x.season, x.average, x.episodes]),
    [[1, 7, 2], [2, 10, 1]]
  );
});

test('o pódio exige um piso de episódios avaliados', async () => {
  const p = await kit.signIn();
  const club = await kit.makeClub({ owner: p.id, visibility: 'public' });
  const magra = show('Uma Vez Só');
  const cheia = show('Com Curva');
  const base = g => ({ showTitle: g.title, showPoster: g.poster, genre: 'Drama' });

  await req('PUT', at(club, `/shows/${magra.id}/1/1`), { ...base(magra), quick: 10 }, p.cookie);
  for (const n of [1, 2, 3]) {
    await req('PUT', at(club, `/shows/${cheia.id}/1/${n}`), { ...base(cheia), quick: 9 }, p.cookie);
  }
  lobbySeries.invalidate();

  const { body } = await req('GET', '/api/lobby/series');
  const nomes = body.podium.map(x => x.title);
  assert.ok(nomes.includes('Com Curva'));
  assert.ok(
    !nomes.includes('Uma Vez Só'),
    'um 10 sobre um episódio não é um ranking, é um entusiasmo'
  );
});
